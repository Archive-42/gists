...
Normally when we set objects equal to `null`, those objects get *garbage
collected* as there is no reference anymore to that object. However,
...
*function* `() => 'greeting'` in this case, and the value
...
parameter to the `get` method. Object interact by *reference*. Functions
...
parameter, namely a *newly* created object `{ ...person }`. This object
...
parameter. Instead, the value of `x` is equal to a *new* object:
...
With the spread operator `...`, we can *spread* iterables to individual
...
(*nullish*), the expression short-circuits and returns `undefined`.
...
`person.pet` is not nullish. `pet` does *not* have a property called
...
their purpose is to *modify* properties. When calling a `setter` method,
...
*awaiting* the value of `item`, the resolved promsie, the resolved
*values* of the promises get returned: `1`, `2`, then `3`.
...
argument. Since we're only passing three separate numeric values (1, 2,
3) instead of one object with properties `x`, `y` and `z` ({x: 1, y: 2,
...
*numeric value* and equal to `NaN`. `name` is not a numeric value, so
...
their initialization: this is called the *temporal dead zone*. In the
...
`finally` block *always* runs, `"Oh finally!"` gets logged.
...
`myPromise` promise, which is an *asynchronous* operation. While the
engine is busy completing the promise, it keeps on running the function
`funcOne`. The next line is the *asynchronous* `setTimeout` function,
...
In `funcTwo` we're, first *awaiting* the myPromise promise. With the
...
The next line is the *asynchronous* `setTimeout` function, from which
...
invoked whenever we *set* property values, `get` gets invoked whenever
we *get* (access) property values.
...
With `Object.seal` we can prevent new properies from being *added*, or
existing properties to be *removed*.
...
The `Object.freeze` method *freezes* an object. No properties can be
added, modified, or removed.

However, it only *shallowly* freezes the object, meaning that only
*direct* properties on the object are frozen. If the property is another
...
The `const` keyword simply means we cannot *redeclare* the value of that
variable, it's *read-only*. However, the value itself isn't immutable.
The properties on the `emojis` array can be modified, for example by
pushing new values, splicing them, or setting the length of the array to
0.
...
promise fails, the `Promise.all` method *rejects* with the value of the
