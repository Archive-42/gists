...
- String `s` _matches_ the regex pattern `/p/` whenever `s` contains
  the pattern 'p'.
  - Example: `abc` matches `/a/`, `/b/`, `/c/`
- For simplicity, we will use the _matches_ verb loosely in a sense
  that
  - a string can _match_ a regex (e.g. 'a' matches /a/)
  - a regex can _match_ a string (e.g. /a/ matches 'a')
- A regex pattern consists of _literal characters_ that match itself,
  and _metasyntax characters_
- Literal characters can be _concatenated_ in a regular expression.
...
- _Alternative execution_ can be achieved with the metasyntax
  character `|`
...
- Matching is _greedy_. A _greedy_ match attempts to stay in
  ...
  - Example: `s = 'baaa'` matches `/a*a/` in the following way:
    ...
    - as there are no more characters in `s` and there is a
      character yet to be matched in the regex, we _backtrack_ one
      character
...
- Backtracking is _minimal_. We attempt to backtrack one character at
...
A regex _literal character_ matches itself. The expression `/a/` matches
